...
function toggleSidebar() {
    ...
    if (isMobile) {
        // Em mobile: toggle overlay + sidebar
        sidebar.classList.toggle('active');
        overlay.classList.toggle('active');
        sidebarOpen = sidebar.classList.contains('active');
    } else {
        // Em desktop: apenas feedback visual (opcional)
        sidebar.classList.toggle('collapsed');
        sidebarOpen = !sidebar.classList.contains('collapsed');
    }
    
    console.log('🎯 Sidebar:', sidebarOpen ? 'Aberta' : 'Fechada');
}
...
function navigateToTab(tabName) {
    ...
    const clickedItem = document.querySelector(`.sidebar-item[data-tab="${tabName}"]`);
    if (clickedItem) {
...
// Atualizar badge de lembretes na sidebar
function updateSidebarReminderBadge() {
    const badge = document.getElementById('sidebarReminderBadge');
    if (!badge || !remindersList) return;
...
function initSidebar() {
    ...
    if (hash) {
        ...
        navigateToTab('dashboard'); // Aba padrão
    }
    
    // Atualizar info do usuário
    updateSidebarUser();
    updateSidebarReminderBadge();
    ...
    // Navegação pelo teclado (ESC fecha sidebar)
...
// Hook nas funções de salvar para atualizar sidebar
const originalSaveAccounts = saveAccounts;
saveAccounts = function() {
    originalSaveAccounts();
    onDataUpdate();
};
...
/* ============================================
   FUNÇÕES AUXILIARES PARA NOVAS ABAS
   ============================================ */

// Dashboard (já existe, só garantir que funciona)
...
// Ações Rápidas (criar se não existir)
function renderQuickActions() {
    // TODO: Implementar se necessário
    // Por enquanto, mostrar as ações rápidas que já existem
    console.log('📌 Ações Rápidas renderizadas');
}

// Recorrentes (já existe no Config, mover para aba própria)
function renderRecurring() {
    // Já tem a função renderRecurringTransactions()
...
// Metas (já existe no Config, mover para aba própria)
function renderGoals() {
    // Já tem a função renderGoalsList()
    if (typeof renderGoalsList === 'function') {
        renderGoalsList();
    }
    console.log('🎯 Metas renderizadas');
}

// Orçamentos (já existe no Config, mover para aba própria)
function renderBudgets() {
    // Já tem a função renderBudgetsList()
    if (typeof renderBudgetsList === 'function') {
        renderBudgetsList();
    }
    console.log('💼 Orçamentos renderizados');
}

// Atalho de teclado para abrir sidebar (Ctrl + B)
document.addEventListener('keydown', (e) => {
...
console.log('✅ Sistema de sidebar carregado!');
console.log('💡 Atalho: Ctrl + B para abrir/fechar sidebar');
